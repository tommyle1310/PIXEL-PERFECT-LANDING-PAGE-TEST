...
const badgeImg = 'https://cdn.shopify.com/s/files/1/0917/5649/5191/files/nysale.png?v=1766822224';
...
class ProductImageSlider {
    ...
    this.container.innerHTML = `
      ...
      <div class="relative flex items-center">
        ...
        <div id="main-slider-container" class="md:hidden w-full aspect-square rounded-xl overflow-hidden bg-gray-200 relative cursor-grab active:cursor-grabbing select-none">
          <div id="main-slider-track" class="flex h-full transition-transform duration-300 ease-out" style="width: 300%; transform: translateX(-33.333%);">
            ...
            <div class="w-1/3 h-full flex-shrink-0">
              <img src="${sliderImages.current}" class="w-full h-full object-cover pointer-events-none" alt="Current" draggable="false">
            </div>
            <div class="w-1/3 h-full flex-shrink-0">
        ...
        <div id="thumbnails-track" class="flex gap-2 transition-transform duration-300 ease-out" style="width: calc(150% + 10px); transform: translateX(calc(-16.666% - 1.333px));">
          ${thumbImages.map((thumb, i) => `
            <div class="aspect-square rounded-lg overflow-hidden cursor-pointer thumbnail-item hover:border-[#039869] transition-all flex-shrink-0" style="width: calc((100% - 45px) / 6);" data-index="${thumb.index}">
              <img src="${thumb.src}" class="w-full h-full object-cover pointer-events-none" alt="Product thumbnail" draggable="false">
            </div>
          `).join('')}
        </div>
      </div>
      ...
      <div class="hidden md:block space-y-3 mt-4">
        <div class="grid grid-cols-2 gap-3">
          ${this.images.slice(1, 3).map((src) => `
            <div class="aspect-square rounded-md overflow-hidden">
              <img src="${src}" class="w-full h-full object-cover" alt="Product thumbnail">
            </div>
          `).join('')}
        </div>
        <div class="grid grid-cols-3 gap-3">
          ${this.images.slice(3, 6).map((src) => `
            <div class="aspect-square rounded-md overflow-hidden">
              <img src="${src}" class="w-full h-full object-cover" alt="Product thumbnail">
            </div>
          `).join('')}
    ...
    const onMove = (e) => {
      ...
      if (type === 'main') {
        ...
        if (track) {
          ...
          track.style.transform = `translateX(calc(${thumbBaseOffset + dragPercent}% - 1.333px))`;
        }
      }
    };
  ...
  animateSlide(direction) {
    ...
    const mainTarget = direction === 'next' ? 'translateX(-66.666%)' : 'translateX(0%)';
    
    // Thumbnail slider: slides by 16.666% (1 thumbnail)
    // Base is -16.666%, next goes to -33.333%, prev goes to 0%
    const thumbTarget = direction === 'next' 
  ...
  goTo(index) {
    ...
    const thumbsContainer = this.container.querySelector('#thumbnails-mobile');
    const mainTrack = mainContainer ? mainContainer.querySelector('#main-slider-track') : null;
    const thumbsTrack = thumbsContainer ? thumbsContainer.querySelector('#thumbnails-track') : null;
    ...
    if (direction === 'next') {
      for (let i = 0; i <= steps; i++) {
        mainImagesArr.push(this.images[this.getWrappedIndex(this.currentIndex + i)]);
      }
    } else {
    ...
    if (mainTrack) {
      ...
      mainTrack.innerHTML = mainImagesArr.map(src => `
        <div class="h-full flex-shrink-0" style="width: ${itemWidthPercent}%;">
          <img src="${src}" class="w-full h-full object-cover pointer-events-none" alt="" draggable="false">
        </div>
      `).join('');
    ...
    if (thumbsTrack) {
      ...
      thumbsTrack.innerHTML = thumbsArr.map(thumb => `
        <div class="aspect-square rounded-lg overflow-hidden cursor-pointer thumbnail-item hover:border-[#039869] transition-colors flex-shrink-0" style="width: calc(${itemWidthPercent}% - ${gapPerItem}px);" data-index="${thumb.index}">
          <img src="${thumb.src}" class="w-full h-full object-cover pointer-events-none" alt="" draggable="false">
        </div>
      `).join('');
      ...
      if (direction === 'next') {
        ...
        thumbsTrack.style.transform = `translateX(-${steps * itemWidthPercent}%)`;
      }
    }
    ...
    setTimeout(() => {
      ...
      if (thumbsTrack) {
        ...
        if (direction === 'next') {
          thumbsTrack.style.transform = `translateX(-${steps * thumbItemWidth}%)`;
        } else {
          thumbsTrack.style.transform = 'translateX(0%)';
        }
      }
